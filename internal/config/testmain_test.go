...
func TestMain(m *testing.M) {
	...
	if err != nil {
		log.Printf("internal/config: TestMain: unable to locate repo root: %v", err)
	} else {
		...
		if fi, err := os.Stat(testBin); err == nil && fi.IsDir() {
			...
			if err := os.Setenv("PATH", newPATH); err != nil {
				log.Printf("internal/config: TestMain: failed to set PATH: %v", err)
			} else {
				log.Printf("internal/config: TestMain: prepended %s to PATH", testBin)
			}
		}
	}

	// Create temporary HOME for tests to avoid interfering with developer environment.
	tmpHome, err := os.MkdirTemp("", "zeroui-internal-config-test-home-")
	if err != nil {
		log.Printf("internal/config: TestMain: failed to create temp HOME dir: %v", err)
	} else {
	...
	defer func() {
		if err := os.Setenv("PATH", origPATH); err != nil {
			log.Printf("internal/config: TestMain: failed to restore PATH: %v", err)
		}
		if hadHOME {
			if err := os.Setenv("HOME", origHOME); err != nil {
				log.Printf("internal/config: TestMain: failed to restore HOME: %v", err)
			}
		} else {
			if err := os.Unsetenv("HOME"); err != nil {
				log.Printf("internal/config: TestMain: failed to unset HOME: %v", err)
			}
		}
		if tmpHome != "" {
			if err := os.RemoveAll(tmpHome); err != nil {
				log.Printf("internal/config: TestMain: failed to remove tmp HOME %s: %v", tmpHome, err)
			}
		}
	}()
