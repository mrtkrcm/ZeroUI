...
### Test Data
- Test binaries in `test/bin/`
- Test configurations in `test/configs/`
- Visual snapshots in `test/snapshots/`
...
### Test Binaries
- Add stub binaries to `test/bin/`
- Make executable: `chmod +x test/bin/*`
- Use in tests via PATH setup

### Test Configurations
- Store test configs in `test/configs/`
- Use relative paths in tests

### Visual Snapshots
- Generated automatically during tests
- Compare with `test/snapshots/` for regression testing
...
1. **PATH issues**: Ensure `test/bin` is in PATH
...
- Check `test/bin/` for missing test binaries
- Verify file permissions with `ls -la test/bin/`
